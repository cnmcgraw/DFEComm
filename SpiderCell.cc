...
void SpiderCell::BuildCell(int Local_ID, int CS_ID, Problem* problem)
{
  ...
  sp_azim.resize(problem->refinement,0);
  sp_azim = problem->sp_azim;
  
  int cells_per_pin(0);
  for(int i = 0; i < sp_azim.size(); i++)
    cells_per_pin += sp_azim[i];
...
void SpiderCell::SetLocalBoundary()
{
...
void SpiderCell::GetCellijk(int Cell_ID, int Dx, int Dy, int Dz, std::vector<int>& ijk)
{
...
void SpiderCell::ComputeCellID(int Local_ID, int CS_ID, Problem* problem)
{
...
void SpiderCell::GetNeighbors(int CS_ID)
{
  // Neighbor Directions
...
void SpiderCell::GetCellInOut(std::vector<double> omega, std::vector<int>& incoming, std::vector<std::vector<int> >& outgoing)
{
...
void SpiderCell::GetFaceNormals()
{
...
void SpiderCell::GetFaceCenters()
{
...
void SpiderCell::GetVertices(int CellID)
{
...
void SpiderCell::ComputeDFEMMatrices()
{
  ...
  // simulate non-Spider geometries
